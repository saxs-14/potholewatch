...
export default function Dashboard() {
  ...
  useEffect(()=>{if(!localStorage.getItem("potholewatch_token") && !import.meta.env.VITE_API_KEY){navigate("/login");return;} const sync=()=>api.flushOfflineReports().then(()=>refresh()).catch(()=>{}); api.health().then(()=>{setOnline(true);sync();}).catch(()=>setOnline(false)); window.addEventListener("online",sync); return ()=>window.removeEventListener("online",sync);},[refresh,navigate]);
  ...
  const submit=async()=>{if(!file)return;setLoading(true);setError(null);try{const r=await api.analyzeImage(file,location,coords?.latitude,coords?.longitude);setSelected(r);setFile(null);setLocation("");setCoords(null);await refresh();}catch(e){try{await api.queueOfflineReport(file,location,coords?.latitude,coords?.longitude);setFile(null);setLocation("");setCoords(null);setError("Connection unavailable. Your report was safely queued on this device and will retry automatically when you reconnect.");}catch{setError(e instanceof Error?e.message:"Analysis failed");}}finally{setLoading(false);}};
